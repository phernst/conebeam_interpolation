...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, filename, neighbor_diff, transform=None):
        self.projections = nib.load(pjoin(data_dir, filename)).get_fdata().transpose(1, 0, 2)
        self.interpolations = nib.load(pjoin(f'interpolations_nd{neighbor_diff}', filename)).get_fdata().transpose(1, 0, 2)
    ...
    def __getitem__(self, idx):
        ...
        projection_plus = self.projections[..., (idx + neighbor_diff) % len(self)]
        projection_minus = self.projections[..., (idx - neighbor_diff) % len(self)]

        sample = {
            'in': np.stack([projection_minus, interpolation, projection_plus], axis=0),
            'out': projection,
        }
...
def main():
    ...
    hparams.neighbor_diff = 2
    hparams.pretrained = False
    hparams.valid_dir = f'valid_ana_nd{hparams.neighbor_diff}' + ('_pre' if hparams.pretrained else '_mse')
    ...
    if hparams.pretrained:
        ...
        pre_dir = f'valid_ana_nd{hparams.neighbor_diff}_mse'
        pre_path = [
